...
class PeliculaService {
  ...
  async obtenerPaginadas({ pagina = 1, limite = 10, genero, buscar }) {
    ...
    if (genero) {
      params.push(genero)
      where += ` WHERE g.slug = $${params.length}`
    }
    ...
    if (buscar) {
      params.push(`%${buscar}%`)
      const condicion = `(p.titulo ILIKE $${params.length} OR d.nombre ILIKE $${params.length})`
      where += where ? ` AND ${condicion}` : ` WHERE ${condicion}`
    }

    // 1️⃣ Obtener total de resultados
    const totalQuery = `
  ...
  async crearResena(peliculaId, datos) {
    ...
    // =====================
  // Calificar película (transacción)
  // =====================
  async calificar(peliculaId, datos) {
    ...
    try {
      ...
      if (puntuacion < 1 || puntuacion > 10) {
        throw new AppError('La puntuación debe estar entre 1 y 10', 400)
      }
      ...
      // 2. Calcular nueva media
      const { rows: mediaRows } = await client.query(
        `SELECT ROUND(AVG(puntuacion)::numeric, 2) AS media
        FROM resenas
        WHERE pelicula_id = $1`,
        [peliculaId]
      )

      const nuevaMedia = mediaRows[0].media

      // 3. Actualizar nota de la película
      await client.query(
        `UPDATE peliculas
        SET nota = $1
        WHERE id = $2`,
        [nuevaMedia, peliculaId]
      )

      await client.query('COMMIT')

      return {
        mensaje: 'Reseña creada y nota actualizada correctamente',
        resena: insertResena.rows[0],
        nuevaNota: nuevaMedia
      }
